...
- [x] Cliente D-Bus BlueZ — Incremento 2: señales y lifecycle (`InterfacesAdded/Removed`, `PropertiesChanged`; suscripción, unsubscribe, cierre) — ✅ **solo el nivel bajo** está implementado y verificado: worker dedicado (`_SignalWorker`, GLib `MainContext`/`MainLoop` daemon), tres filtros exactos, `SignalEvent` (metadata, sin payload), suscripción/unsubscribe/close idempotentes con cero callbacks tras el unsubscribe, arranque perezoso/restart, timeout y rollback atómico; TDD determinista (sin GI) + integración real de **lifecycle** en Python 3.12/Gio (25 ciclos subscribe/unsubscribe/close + snapshot fresco, sin cerrar el bus, sin inducir señales). Contrato técnico en [signal-lifecycle-design](bluez/signal-lifecycle-design.md). ⏳ **Lo pendiente es el dispatch del repositorio** (`subscribe_device_changes`: cache de diff, orden determinista y `Unsubscribe`), que se cierra con `IBluetoothRepository` (siguiente ítem). El contrato de eventos del dominio ([ADR-0007](ADR/0007-device-change-event-contract.md): `DeviceChangeKind`/`DeviceChangeEvent`/`Unsubscribe`) está **preparado e implementado**
- [x] Mapeo de objetos D-Bus → modelos (`bluez/object_mapper.py`)
- [ ] Implementación de `IBluetoothRepository` (`bluez/bluez_repository.py`) — **consultas snapshot implementadas y verificadas** (`list_adapters`/`list_devices`/`get_device`/`get_battery`/`get_rssi`, cliente inyectable + snapshot fresco; TDD e integración real solo lectura completados). El checkbox **sigue vacío**: `subscribe_device_changes` sigue lanzando `NotImplementedError`. El **nivel bajo de señales** del Incremento 2 ya está implementado (worker, `SignalEvent`, suscripción/unsubscribe/close), pero **falta el dispatch del repositorio** (registro de suscriptores, cache de diff, cierre de carrera A→B, orden determinista REMOVED→ADDED→UPDATED y `Unsubscribe` idempotente; contrato en [signal-lifecycle-design §4](bluez/signal-lifecycle-design.md#4-repositorio-registro-cache-y-dispatch), eventos del dominio [ADR-0007](ADR/0007-device-change-event-contract.md) ya listos y probados)
